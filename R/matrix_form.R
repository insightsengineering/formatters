...
mform_handle_newlines <- function(matform, has_topleft = TRUE) {
  # row_nlines <- apply(matform$strings, 1, nlines) # nolint
  ...
  nr_header <- mf_nrheader(matform) # attr(matform, "nrow_header") # nolint
  ...
  if (any(row_nlines > 1)) {
    ...
    # matform$spans <- expand_mat_rows(matform$spans, row_nlines, rep_vec_to_len) # nolint
    mf_spans(matform) <- expand_mat_rows(mf_spans(matform), row_nlines, rep_vec_to_len)
    # matform$aligns <- expand_mat_rows(matform$aligns, row_nlines, rep_vec_to_len) # nolint
    mf_aligns(matform) <- expand_mat_rows(mf_aligns(matform), row_nlines, rep_vec_to_len)
    # matform$display <- expand_mat_rows(matform$display, row_nlines, rep_vec_to_len) # nolint
    mf_display(matform) <- expand_mat_rows(mf_display(matform), row_nlines, rep_vec_to_len)
    # matform$line_grouping <- rep(1:nrows, times = row_nlines) # nolint
    mf_lgrouping(matform) <- rep(1:nrows, times = row_nlines)

    # attr(matform, "nlines_header") <- sum(row_nlines[1:nr_header]) # nolint
...
#' }
#'
MatrixPrintForm <- function(strings = NULL, # nolint
                            spans,
                            aligns,
                            formats,
                            #    display,
