...
#'
#' @keywords internal
#' A function factory to generate xx-style `rtables` formatting functions
#' 2 different rounding methods are considered, R and SAS rounding
#'
format_xx_fct <- function(round_method = c("R", "SAS")) {
  ...
  fnct <- function(str) {
    ...
    rtable_format <- function(x, output, na_str = "NA") {
      ...
      if ((length(x) > 1) && length(na_str) < length(x)) {
      ...
      for (i in seq_along(x)) {
        values[[i]] <- roundings[[i]](x[[i]], na_str = na_str[[i]])
      }
...
#' format_value(c(1.453, NA), format_xx_r("xx.xx (xx.xxx)"))
#' @export
...
format_xx_count_fraction_fct <- function(
    round_method = c("SAS", "R"),
    type = c("count_fraction", "count_denom_fraction")) {
  round_method <- match.arg(round_method)
  type <- match.arg(type)
...
#' @description A family of formatting functions for presenting a count together with fraction (and denominator) with
#' special consideration when count is 0, or fraction is 1.
...
#'
#' @return A string in the format `count / denom (ratio %)`. If `count` is 0, the format is `0`.
#' If fraction is >0.99, the format is `count / denom (>99.9%)`
#' @export
#' @examples
