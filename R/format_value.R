...
#'
#' @export
#' @return A formatting function which wraps and will apply the specified \code{printf} style
...
#' @param na_str character(1). String that should be displayed when the value of \code{x} is
#'   missing. Defaults to \code{"NA"}.
...
format_value <- function(x, format = NULL, output = c("ascii", "html"), na_str = "NA") {
    ...
    txt <- if (all(is.na(x)) && length(na_str) == 1L) {
        ...
        switch(format,
            ...
            "xx.xx (xx.xx - xx.xx)" = paste0(
                ...
                sep_2d_helper(x[2:3], dig1 = 2, dig2 = 2, sep = " - ", na_str = na_str[2:3], wrap = c("(", ")"))
            ),
            "xx. (xx. - xx.)" = paste0(
                round_fmt(x[1], digits = 0, na_str = na_str[1]), " ",
                sep_2d_helper(x[2:3], dig1 = 0, dig2 = 0, sep = " - ", na_str = na_str[2:3], wrap = c("(", ")"))
            ),
            "xx.x (xx.x - xx.x)" = paste0(
                round_fmt(x[1], digits = 1, na_str = na_str[1]), " ",
                sep_2d_helper(x[2:3], dig1 = 1, dig2 = 1, sep = " - ", na_str = na_str[2:3], wrap = c("(", ")"))
            ),
            "xx.xx (xx.xx - xx.xx)" = paste0(
                round_fmt(x[1], digits = 2, na_str = na_str[1]), " ",
                sep_2d_helper(x[2:3], dig1 = 2, dig2 = 2, sep = " - ", na_str = na_str[2:3], wrap = c("(", ")"))
            ...
            "xx.xxx (xx.xxx - xx.xxx)" = paste0(
                ...
                sep_2d_helper(x[2:3], dig1 = 3, dig2 = 3, sep = " - ", na_str = na_str[2:3], wrap = c("(", ")"))
            ),
    ...
    if (output == "ascii") {
        ...
        if (identical(txt, "")) {
            ...
            Map(function(el, is.last) { # nolint
